						    	...
						    	<ul class="list-group filenames my-3">
						    		...
						    		<c:forEach items="${board.attachs}" var="attach">
							    		...
							    		<a href="/download${attach.url}&thumb=off">
							    			<i class="far fa-file fa-spin"></i> ${attach.name}
							    		</a> 
							    		<%-- <i class="far fa-times-circle btn-remove float-right" data-file="name=${attach.name}&amp;path=${attach.path}&amp;uuid=${attach.uuid}&amp;image=${attach.image}&amp;thumb=off"></i> --%> 
						    		</li>
						    		</c:forEach>
						    	</ul>
						    	<ul class="nav thumbs">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                	<div class="form-group">
					    ...
					    <input type="text" class="form-control" placeholder="Enter reply" id="reply">
					</div>
					<div class="form-group">
					    <label for="replyer">Replyer</label>
					    <input type="text" class="form-control" placeholder="Enter replyer" id="replyer" readonly>
					</div>
					<div class="form-group">
...
$(function() {
	...
	/* 이름이고 변수 식별자{bno:bno}
	replyService.getList({bno:bno, rno:25}, function(result) {
		console.log(result);
	}) */
	/* replyService.get(26, function(result) {
		console.log(result);
	});
	replyService.remove(211, function(result) {
		console.log(result);
	})
	 */
/* 	replyService.modify({rno:210, reply:'수정된 댓글 내용'}, function(result) {
	...
	function showUploadedFile(uploadResultArr) {
		...
		for(var i in uploadResultArr) {
			...
			str += `<li class="list-group-item" data-uuid="\${obj.uuid}" data-name="\${obj.name}" data-path="\${obj.path}" data-image="\${obj.image}">
					<a href="/download\${obj.url}"><i class="far fa-file fa-spin"></i>`;
