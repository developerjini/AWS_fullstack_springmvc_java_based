...
import com.developerjini.domain.SampleVO;
 ...
 * controller ?
 * 요청과 응답 처리 제어
 * request : 1. URL(포트 이후) 2. 파라미터 수집(쿼리스트링(get), post(request의 헤더)) 3. HTTP Method(GET, POST, PUT, DELETE) .. attribute, session, cookie 
 * response : 1. MIME-TYPE(기본값 : text/html(JSP 가장 많이 쓰는 타입), 스프링에서는 뷰리졸버) cf. forward, viewResolver, 2. header(text/html, application/json, text/xml, application/octet-stream(파일다운로드시))
 * 컨트롤러가 void면 동일한 이름의 jsp 호출
*/
